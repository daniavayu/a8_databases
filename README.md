...
In Part 1, the PubMed XML file was processed to extract each article’s PMID, title, and abstract. The abstracts were cleaned by lowercasing, removing punctuation, removing numbers, removing stopwords, and lemmatizing words. Then, a ChromaDB vector database was created. Each abstract was converted into an embedding using ChromaDB’s default embedding model and inserted into the database in batches to make the loading process more efficient and reliable.
